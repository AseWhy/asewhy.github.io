...
});
{
  ...
  function isExists(docname) {
    return new Promise(res => {
      fetch('./frames/' + docname + '.htm').catch(() => res(false)).then(e => {
        res(e.status == 200);
      });
    });
  }
  ...
  async function message(message) {
    ...
    if (data.sender === w_id) {
      switch (data.command) {
        case 'open.link':
          if (await window.root.notify('Переход по ссылке', 'Вы уверены, что хотите перейти на \'' + data.data + '\'?', 'Да', 'Отмена')) window.location = data.data;
          break;

        case 'header.hide':
          $('#div-block-target').css({
            opacity: 0,
            height: 0
          });
          break;
        ...
        case 'header.buttons.add':
          {
            ...
            if (container.children.length > 0) container.appendChild(sepo);
        ...
        case 'header.update.title':
          $('#target-header-title').text(data.data.title).attr('href', data.data.url);
          break;

        case 'header.update.logo':
          $('#target-header-logo').attr('src', './frames/' + data.data);
          break;
...
}
{
  $(window).on('click', e => {
    if (e.target.tagName == 'A') {
      (async () => {
        if (await window.root.notify('Переход по ссылке', 'Вы уверены, что хотите перейти на \'' + e.target.href + '\'?', 'Да', 'Отмена')) window.location = e.target.href;
      })();
